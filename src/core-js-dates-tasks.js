...
function getNextFriday(date) {
  ...
  if (currentDay === 5) {
    date.setDate(dateOfMonth + 7);
  } else if (currentDay < 5) {
    const leftDays = 5 - currentDay;
    date.setDate(dateOfMonth + leftDays);
  } else {
    const leftDays = 5 - currentDay + 7;
    date.setDate(dateOfMonth + leftDays);
  }
  return date;
}
...
function getCountDaysInMonth(month, year) {
  const lastDatePreviousMonth = new Date(year, month, 0);
  const countofdays = lastDatePreviousMonth.getDate();
  return countofdays;
}
...
function formatDate(date) {
  ...
  const hrResult = Number(hr) > 12 ? Number(hr) - 12 : hr;
  const mins = dateObj.getMinutes();
  const secs = dateObj.getSeconds();
  const month = dateObj.getMonth() + 1;
  const day = dateObj.getUTCDate();
  const year = dateObj.getFullYear();
  const time = Number(hr) >= 12 ? 'PM' : 'AM';
  return `${month}/${day}/${year}, ${hrResult}:${mins.toString().padStart(2, '0')}:${secs.toString().padStart(2, '0')} ${time.toString().padStart(2, '0')}`;
}
